...
def fetch_data(cache_file_name, verify=False, cache_directory_name=""):
    """
...
def make_isotropic(image, interpolator=SimpleITK.sitkLinear):
    original_spacing = image.GetSpacing()
    print('original_spacing', original_spacing)
    if all(spc == original_spacing[0] for spc in original_spacing):
        return SimpleITK.Image(image)
    # Make image isotropic via re-sampling
    ...
    return SimpleITK.Resample(image, new_size, SimpleITK.Transform(), interpolator, image.GetOrigin(), new_spacing,
                              image.GetDirection(), 0, image.GetPixelID())


img1 = SimpleITK.ReadImage(fetch_data('training_001_mr_T1.mha'))
img2_res = SimpleITK.ReadImage(fetch_data('training_001_ct.mha'))
print('img1', img1.GetSize(), img1.GetSpacing())
# print('img2', img2.GetSize(), img2.GetSpacing())
img2 = SimpleITK.Resample(img2_res, img1)
print('img2_res', img2_res.GetSize(), img2_res.GetSpacing())
gui.MultiImageDisplay(image_list=[img1, img2, img2_res],
                      title_list=['img1', 'img2', 'img2_res'],
                      figure_size=(12, 3))


arr1 = SimpleITK.GetArrayFromImage(img1)
arr2_res = SimpleITK.GetArrayFromImage(img2)
arr2 = SimpleITK.GetArrayFromImage(img2_res)
print('arr1', numpy.max(arr1), numpy.min(arr1))
print('arr2', numpy.max(arr2), numpy.min(arr2))
print('arr2_res', numpy.max(arr2_res), numpy.min(arr2_res))
img1_255 = SimpleITK.Cast(SimpleITK.IntensityWindowing(img1, windowMinimum=2, windowMaximum=1626, outputMinimum=0.0, outputMaximum=255.0), SimpleITK.sitkUInt8)
img2_255 = SimpleITK.Cast(SimpleITK.IntensityWindowing(img2, windowMinimum=-1024, windowMaximum=1945, outputMinimum=0.0, outputMaximum=255.0), SimpleITK.sitkUInt8)
gui.MultiImageDisplay(image_list=[img1_255, img2_255],
                      title_list=['img1_255', 'img2_255'],
                      figure_size=(8, 4))


msk1 = SimpleITK.OtsuThreshold(img1, 0, 1)
msk2 = SimpleITK.OtsuThreshold(img2, 0, 1)

gui.MultiImageDisplay(image_list=[msk1, msk2],
                      title_list=['msk1', 'msk2'],
                      figure_size=(8, 4))
...
def mask_image_multiply(mask, image):
    ...
    if components_per_pixel == 1:
        ...
        return SimpleITK.Compose([mask * SimpleITK.VectorIndexSelectionCast(image, channel) for channel in range(components_per_pixel)])


def alpha_bend(image1, image2, alpha=0.5, mask1=None, mask2=None):
    if not mask1:
        mask1 = SimpleITK.Image(image1.GetSize(), SimpleITK.sitkFloat32) + 1.0
        mask1.CopyInformation(image1)
    else:
        SimpleITK.Cast(mask1, SimpleITK.sitkFloat32)
    if not mask2:
        mask2 = SimpleITK.Image(image2.GetSize(), SimpleITK.sitkFloat32) + 1.0
        mask2.CopyInformation(image2)
    else:
        SimpleITK.Cast(mask2, SimpleITK.sitkFloat32)

    components_per_pixel = image1.GetNumberOfCompnentsPerPixel()
    if components_per_pixel > 1:
        img1 = SimpleITK.Cast(image1, SimpleITK.sitkVectorFloat32)
        img2 = SimpleITK.Cast(image2, SimpleITK.sitkVectorFloat32)
    else:
        img1 = SimpleITK.Cast(image1, SimpleITK.sitkFloat32)
        img2 = SimpleITK.Cast(image2, SimpleITK.sitkFloat32)
...
zeros = SimpleITK.Image(img1_255.GetSize(), img1_255.GetPixelID())
zeros.CopyInformation(img1_255)

gui.MultiImageDisplay(image_list=[SimpleITK.Cast(SimpleITK.Compose(img1_255, img2_255, zeros), SimpleITK.sitkVectorUInt8),
                                  SimpleITK.Cast(SimpleITK.Compose(img1_255, img2_255, img1_255), SimpleITK.sitkVectorUInt8),
                                  SimpleITK.Cast(SimpleITK.Compose(img1_255, 0.5 * img1_255 + 0.5 * img2_255, img2_255), SimpleITK.sitkVectorUInt8)],
                      title_list=['avoid red_green', 'use magenta-green', 'use orange-blue'],
                      figure_size=(9, 3))
